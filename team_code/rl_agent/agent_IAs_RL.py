...
class AgentIAsRL(BaseAgent):
    ...
    def run_step(self, observations, timestamp):
        # rgb->image, velocity->speed, command->far_command
        self.current_timestep += 1
        rgb = observations["image"].copy()
        ...
        speed = np.linalg.norm(observations["speed"])

        order = self.adapt_order(int(observations["far_command"]))
